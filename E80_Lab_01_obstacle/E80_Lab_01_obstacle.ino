/********
Default E80 Code modified for obstacle course
Current Author:
...
void loop() {
  ...
  // If robot facing away from you, Motor A = left, Motor B = right, Motor C = vert
  if (currentTime > 60000 && currentTime < 66000) { // Wait 1 min to deploy
    motorDriver.drive(0,0,-100);   // Go down for 6 sec
  } else if (currentTime > 66000 && currentTime < 73000) {
    motorDriver.drive(-190,-190,0);    // Go forward for 7 sec
  } else if (currentTime > 73000 && currentTime < 78000) {
    motorDriver.drive(0,0,255);   // Go up for 5 sec
  } else {
